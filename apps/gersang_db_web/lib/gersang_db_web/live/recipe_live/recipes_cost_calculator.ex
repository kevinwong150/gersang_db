defmodule GersangDbWeb.RecipeLive.RecipesCostCalculator do
  ...
  def update(assigns, socket) do
    ...
    socket =
      ...
      # Initialize highlighted_item_id
      |> assign_new(:highlighted_item_id, fn -> nil end)
      # Initialize related_item_ids
      |> assign_new(:related_item_ids, fn -> %{} end)
  ...
  def handle_event("update_ingredient_cost", %{"item-id" => item_id, "value" => cost}, socket) do
    ...
    parsed_cost =
      case cost do
        ...
        cost_num when is_number(cost_num) ->
          trunc(cost_num)
    ...
    updated_ingredient_costs =
      Map.put(socket.assigns.ingredient_costs, String.to_integer(item_id), parsed_cost)
  ...
  def handle_event("toggle_node", %{"node-id" => node_id}, socket) do
    node_id_int = String.to_integer(node_id)
    expanded_nodes = socket.assigns.expanded_nodes

    updated_expanded_nodes =
      if Map.get(expanded_nodes, node_id_int, true) do
        Map.put(expanded_nodes, node_id_int, false)
      else
        Map.put(expanded_nodes, node_id_int, true)
      end

    {:noreply, assign(socket, :expanded_nodes, updated_expanded_nodes)}
  end

  def handle_event("expand_all", %{"node-id" => node_id}, socket) do
    node_id_int = String.to_integer(node_id)
    expanded_nodes = socket.assigns.expanded_nodes

    # Get all descendant node IDs for this node
    descendant_node_ids = get_all_descendant_node_ids(node_id_int, socket.assigns.materials_tree)

    # Set all descendants to expanded
    updated_expanded_nodes =
      Enum.reduce([node_id_int | descendant_node_ids], expanded_nodes, fn id, acc ->
        Map.put(acc, id, true)
      end)
  ...
  def handle_event("collapse_all", %{"node-id" => node_id}, socket) do
    node_id_int = String.to_integer(node_id)
    expanded_nodes = socket.assigns.expanded_nodes
    ...
    updated_expanded_nodes =
      Enum.reduce(descendant_node_ids, expanded_nodes, fn id, acc ->
  ...
  def handle_event("update_cost", %{"item-id" => item_id}, socket) do
    ...
    # Find the root node (should be in layer 0) for this item
    root_node =
      socket.assigns.materials_tree
      |> Map.get("0", [])
      |> Enum.find(fn %{item: item} -> item.id == item_id_int end)

    if root_node do
      # Calculate the total cost using the root node's node_id
      total_cost =
        calculate_materials_cost(
          root_node.node_id,
          socket.assigns.materials_tree,
          socket.assigns.ingredient_costs
        )
      ...
      case GersangDb.GersangItem.update_item(item, %{
        ...
        {:ok, updated_item} ->
          ...
          updated_materials_tree =
            update_item_in_materials_tree(socket.assigns.materials_tree, updated_item)
          ...
          socket =
            ...
            |> put_flash(
              :info,
              "Cost updated successfully! Total cost: #{ViewHelpers.format_number_with_commas(total_cost)}, Margin: #{Float.round(margin, 2)}%"
            )
  ...
  def build_materials_tree(%GersangDb.Domain.GersangItem{} = product) do
    layer = 0
    # Start with a unique node ID for the root product
  ...
  defp build_materials_tree(materials_with_parents, layer, tree)
       when is_list(materials_with_parents) and is_map(tree) do
    base_tree = Map.put(tree, "#{layer}", materials_with_parents)

    next_layer_materials_with_parents =
      list_next_layer_materials_with_parent(materials_with_parents)
  ...
  def list_next_layer_materials_with_parent(materials_with_parents) do
    ...
    |> Enum.flat_map(fn %{item: item, node_id: parent_node_id} ->
      item.materials
      |> Enum.map(fn material ->
        %{
          node_id: generate_node_id(),
  ...
  # Generate a unique node ID using a combination of timestamp and random value
  ...
  def get_item_production_fee(%GersangDb.Domain.GersangItem{} = item) do
    ...
    # Return the maximum production fee from all recipes, or 0 if none
    |> Enum.max(fn -> 0 end)
  end
  ...
  def get_item_production_amount(%GersangDb.Domain.GersangItem{} = item) do
    ...
    # Return the maximum production amount from all recipes, or 1 if none
    |> Enum.max(fn -> 1 end)
  end

  def get_material_amount_from_recipe(
        %GersangDb.Domain.GersangItem{} = product,
        %GersangDb.Domain.GersangItem{} = material
      ) do
    product
    |> Map.get(:recipes_as_product, [])
    |> Enum.find(fn recipe ->
    ...
    |> then(fn
      # Default amount if no recipe found
      nil -> 1
      # Use the recipe's amount or default to 1
      recipe -> recipe.material_amount || 1
    end)
  end
  ...
  def material_tree_node(assigns) do
    children = find_children(assigns.node.node_id, assigns.layer_index + 1, assigns.all_nodes)
    has_children = !Enum.empty?(children)
    is_expanded = Map.get(assigns.expanded_nodes, assigns.node.node_id, true)
    ...
    <div
      ...
      id={"node-" <> Integer.to_string(@node.node_id)}
    >
      <div class="mb-4">
          ...
          <div class="flex items-center gap-3">
            <%= if @has_children do %>
              ...
              <button
                ...
                phx-value-node-id={@node.node_id}
              ...
              <div class="flex items-center gap-2">
                ...
                <%= if @has_children do %>
                  ...
                  <div class="ml-auto px-3 py-1.5 bg-gradient-to-r from-emerald-100 to-green-100 border-2 border-emerald-300 rounded-lg shadow-sm">
                    ...
                    💰 <%=
                        total_cost = calculate_materials_cost(@node.node_id, @all_nodes, @ingredient_costs)
  ...
  attr(:parent_node_id, :integer, required: true)
  attr(:all_nodes, :map, required: true)
  attr(:ingredient_costs, :map, default: %{})
  attr(:target_handler, :any, required: true)
  ...
  def cost_breakdown_formula(assigns) do
    breakdown_data =
      calculate_materials_cost_breakdown(
        assigns.parent_node_id,
        assigns.all_nodes,
        assigns.ingredient_costs
      )
  ...
  defp calculate_materials_cost(node_id, all_nodes, ingredient_costs) do
    # Find the node by its node_id
    node = get_node_by_id(node_id, all_nodes)

    if is_nil(node) do
      0
    else
      # Find all direct materials (children) of this node
      children = find_children(node_id, get_node_layer(node_id, all_nodes) + 1, all_nodes)

      if Enum.empty?(children) do
        # No materials, so cost is 0
        0
      else
        # Calculate materials cost
        materials_total =
          children
          |> Enum.reduce(0, fn %{item: child_item, amount: amount, node_id: child_node_id}, acc ->
            # Check if this child has its own materials (is a parent node)
            child_has_materials =
              !Enum.empty?(
                find_children(
                  child_node_id,
                  get_node_layer(child_node_id, all_nodes) + 1,
                  all_nodes
                )
              )

            child_cost =
              if child_has_materials do
                # If child has materials, use its calculated cost (recursive)
                calculate_materials_cost(child_node_id, all_nodes, ingredient_costs)
              else
        ...
        # Get the current node to access its production info
        production_fee = (node && node[:production_fee]) || 0
        # Apply the formula: (materials_total + production_fee) / production_amount
        production_amount = (node && node[:production_amount]) || 1
  ...
  defp calculate_materials_cost_breakdown(node_id, all_nodes, ingredient_costs) do
    # Find the node by its node_id
    node = get_node_by_id(node_id, all_nodes)

    if is_nil(node) do
      {0, "Node not found", [], 0, 1}
    else
      # Find all direct materials (children) of this node
      children = find_children(node_id, get_node_layer(node_id, all_nodes) + 1, all_nodes)
      ...
      if Enum.empty?(children) do
        ...
        materials_for_costing =
          children
          |> Enum.map(fn %{item: child_item, amount: amount, node_id: child_node_id} ->
            # Check if this child has its own materials (is a parent node)
            child_has_materials =
              !Enum.empty?(
                find_children(
          ...
          |> Enum.map(fn {_item_id, materials} ->
            ...
            total_amount =
              Enum.reduce(materials, 0, fn %{amount: amount}, acc -> acc + amount end)

            total_price =
              Enum.reduce(materials, 0, fn %{total_price: price}, acc -> acc + price end)
        ...
        case materials_for_costing do
          ...
          priced_materials ->
            materials_total =
              Enum.reduce(priced_materials, 0, fn material, acc -> acc + material.total_price end)
  ...
  # Helper function to get a node by its node_id
  defp get_node_by_id(node_id, all_nodes) do
    all_nodes
    |> Enum.flat_map(fn {_layer, nodes} -> nodes end)
    |> Enum.find(fn %{node_id: id} -> id == node_id end)
  end

  # Helper function to get the layer of a node by node_id
  defp get_node_layer(node_id, all_nodes) do
    all_nodes
    |> Enum.find_value(fn {layer_str, nodes} ->
      if Enum.any?(nodes, &(&1.node_id == node_id)) do
  ...
  defp find_children(parent_node_id, child_layer_index, all_nodes) do
    child_layer_key = Integer.to_string(child_layer_index)

    case Map.get(all_nodes, child_layer_key) do
      nil -> []
      nodes -> Enum.filter(nodes, &(&1.parent_node_id == parent_node_id))
    end
  end
  ...
  defp get_highlight_class(current_node_id, highlighted_item_id, related_ids_map) do
    cond do
      current_node_id == highlighted_item_id ->
        # Highlight for the item itself (mouseover target)
      ...
      highlighted_item_id && Map.has_key?(related_ids_map, highlighted_item_id) ->
        # Fetch the list of related IDs
        related_ids = related_ids_map[highlighted_item_id]

        if is_list(related_ids) && Enum.member?(related_ids, current_node_id) do
          # Highlight for related items
          "bg-green-50 border-green-400 shadow-md ring-1 ring-green-200"
        else
          # Default if not related
          "bg-gradient-to-r from-slate-50 to-gray-50 hover:from-indigo-50 hover:to-purple-50 border-slate-300"
        end
  ...
  defp get_item_name_styling(layer_index, has_children) do
    base_classes =
      if has_children do
        # Parent items are more prominent
        "font-bold text-lg tracking-wide"
      else
        # Leaf items are less prominent but still bold
  ...
  defp initialize_expanded_nodes(materials_tree) do
    materials_tree
    |> Enum.flat_map(fn {_layer, nodes} -> nodes end)
    |> Enum.reduce(%{}, fn %{node_id: node_id}, acc ->
      Map.put(acc, node_id, true)
    end)
  end
  ...
  # Helper function to get all descendant node IDs for a given node
  defp get_all_descendant_node_ids(node_id, materials_tree) do
    node_layer = get_node_layer(node_id, materials_tree)
    get_descendants_recursive_by_node(node_id, node_layer + 1, materials_tree, [])
  end

  defp get_descendants_recursive_by_node(parent_node_id, layer, materials_tree, acc) do
    children = find_children(parent_node_id, layer, materials_tree)

    if Enum.empty?(children) do
      acc
    else
      child_node_ids = Enum.map(children, & &1.node_id)
      new_acc = acc ++ child_node_ids

      # Recursively get descendants of each child
      Enum.reduce(children, new_acc, fn %{node_id: child_node_id}, current_acc ->
        get_descendants_recursive_by_node(child_node_id, layer + 1, materials_tree, current_acc)
      end)
    end
  end
